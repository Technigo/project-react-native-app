...
const isShaking = (data) => {
  ...
  const handleShake = (event) => {
    setTotalForce(Math.abs(data.x.toFixed(2)) + Math.abs(data.y.toFixed(2)) + Math.abs(data.z.toFixed(2)))
  }

  // setTotalForce(Math.abs(data.x) + Math.abs(data.y) + Math.abs(data.z))
  // const totalForce = Math.abs(data.x) + Math.abs(data.y) + Math.abs(data.z);

  // If this force exceeds some threshold, return true, otherwise false
  // Increase this threshold if you need your user to shake harder
  if (totalForce > 1.5) {
  return(<Text>SHAKE HARDER!</Text>)
  } else if (totalForce > 2) {
  return(<Text>SHAKE HARDER!!!!!</Text>)
  } else if (totalForce > 2.5) {
    return (<Text>SHAKE HARDER!!!!!!!!!!!!!!</Text>)
  }
};
